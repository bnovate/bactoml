...
class GraphModel(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, columns, cc_threshold=0.2, distance_func=distance.euclidean, **kwargs):
        """
        ...
        self.__dict__.update((k, v) for k, v in kwargs.items() if k == 'p')
    ...
    def initialize_graph(self, X):
        ...
        def get_mean_distance(row, df):
            ...
            for _, vals in df.iterrows():
                if 'p' in self.__dict__.keys():
                    mean_distance += self.distance_func(vals.values, row.values, p = self.p)
                else:
        ...
        if 'p' in self.__dict__.keys():
            edges = filter(lambda x: self.distance_func(x[0][1]['features'], x[1][1]['features'], p = self.p) < self.graph.graph['dist_threshold'], itertools.combinations(self.graph.nodes(data=True), 2))
        else:
    ...
    def transform(self, X, y=None):
        ...
        for node, idx in zip(self.graph.nodes(data=True), range(length_i)):
            if 'p' in self.__dict__.keys():
                dist = self.distance_func(self.graph.nodes(data=True)[new_idx]['features'], node[1]['features'], p = self.p)
            else:
